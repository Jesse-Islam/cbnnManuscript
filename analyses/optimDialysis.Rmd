...
```{r dialysisPrep,eval=FALSE,hide=FALSE}




dialysis<-readRDS("dialysis.rds")


data <- dialysis[, c(1,2,3,4,7,6,5)]
colnames(data)[7]<-"status"

#RcmdrPlugin.survival/html/Dialysis.html
...
smallCenters<-as.numeric(names(which( table(data$center)<30)))
toRemove<-c()
for(i in 1:nrow(data)){
  if(data$center[i] %in% smallCenters){
...
preData<-data.frame(#center=as.factor(data$center),
                    age=as.numeric(data$age),
                    #begin=as.numeric(data$begin),
                    disease=as.factor(data$disease),
                    time=data$time,
                    status=data$status)
...
saveRDS(res$test,"dialysisTest.rds")
...
saveRDS(winners,"dialysisWins.rds")

```

```{r bestFits,finalFit}

winners<-readRDS("dialysisWins.rds")
...
while (pos <= iteration) {
  ...
  fullTest<-readRDS("dialysisTest.rds")
  ...
  saveRDS(BrierIPAList,'results/100dialysisBrier.rds')
  saveRDS(cScore,'results/100dialysiscidx.rds')
  pos=pos+1
...
BrierIPAList<-readRDS('results/100dialysisBrier.rds')
...
cidxList<-readRDS('results/100dialysiscidx.rds')
