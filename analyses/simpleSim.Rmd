...
layer1<-3*15#as.numeric(params$layer1)
layer2<-3*15#as.numeric(params$layer2)
...
train<-normalizer(train,means,sds)
  ...
  ##########################
  source_python('src/dsmScript.py')
  dsmSurv<-dsmfitter(train=train,fullTest=test,bsize=floor(nrow(train)/100),epochs=20,val=val,times=times,tPos=ncol(train)-2,ePos=ncol(train)-1,lay1=layer1,lay2=layer2,lay3=layer3,lay4=layer4,lr=lr)
  if((sum(is.na(dsmSurv))==ncol(dsmSurv)*nrow(dsmSurv))){
    
  dsmSurv<-dsmfitter(train=train,fullTest=fullTest,bsize=floor(nrow(train)/100),epochs=epo,val=val,times=times,tPos=ncol(train)-2,ePos=ncol(train)-1,lay1=layer1,lay2=layer2,lay3=layer3,lay4=layer4,lr=lr)
...
```{r}
  ...
  geom_line()+coord_cartesian(ylim=c(-.5,.5))
