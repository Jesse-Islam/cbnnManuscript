...
aids<-aids[which(aids$T.categ=="hs"),]
aids<-aids[which(aids$sex=="M"),]


stattt<-as.factor(aids$status)


levels(stattt)<-c(0,1)
preData<-data.frame(#sex=as.factor(aids$sex),
                    #tcat=as.factor(aids$T.categ),
                    age=as.numeric(aids$age),
                    time=as.numeric(as.numeric(aids$death)-as.numeric(aids$diag)),
                    status=as.numeric(stattt)-1
)
...
saveRDS(winners,"aidsWins.rds")

```

```{r bestFits,finalFit}

winners<-readRDS("aidsWins.rds")
...
while (pos <= iteration) {
  ...
  saveRDS(BrierIPAList,'results/100aidsBrier.rds')
  saveRDS(cScore,'results/100aidscidx.rds')
  pos=pos+1
...
BrierIPAList<-readRDS('results/100aidsBrier.rds')
