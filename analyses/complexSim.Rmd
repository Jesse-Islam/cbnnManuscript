...
layer1<-3*7#as.numeric(params$layer1)
layer2<-3*6#as.numeric(params$layer2)
layer3<-3*5#as.numeric(params$layer3)
layer4<-as.numeric(params$layer4)
drpt<-as.numeric(params$drpt)
lr=as.numeric(params$lr)
paramTrials<-readRDS("complexTrials.rds") 
lr<-paramTrials[which.max(paramTrials$integBrier),]$lr
layer1<-paramTrials[which.max(paramTrials$integBrier),]$layer1
layer2<-paramTrials[which.max(paramTrials$integBrier),]$layer2
layer3<-3*5#as.numeric(params$layer3)
layer4<-as.numeric(params$layer4)
drpt<-paramTrials[which.max(paramTrials$integBrier),]$drpt
bsize=paramTrials[which.max(paramTrials$integBrier),]$bsize
activ<-paramTrials[which.max(paramTrials$integBrier),]$actv


lr<-paramTrials[which.min(paramTrials$valLoss),]$lr
layer1<-paramTrials[which.min(paramTrials$valLoss),]$layer1
layer2<-paramTrials[which.min(paramTrials$valLoss),]$layer2
layer3<-3*5#as.numeric(params$layer3)
layer4<-as.numeric(params$layer4)
drpt<-paramTrials[which.min(paramTrials$valLoss),]$drpt
bsize=paramTrials[which.min(paramTrials$valLoss),]$bsize
activ<-paramTrials[which.min(paramTrials$valLoss),]$actv
...
coefficients<-c(coefficients,z1=0.05,z2=1.05,z3=1.05,term1=1.95,term2=1.05,term3=1.05)
...
while (pos <= iteration) {
  ...
  dat <- simsurv(betas = coefficients, # "true" parameter values
                 ...
                 interval = c(0,10000000)) # interval for root finding
  colnames(dat)<-c("id","time","status")
  samp<-sample(seq(1,nrow(dat),by=1), floor(nrow(dat)*(0.05))) #random censoring
  #dat$status[samp]<-0
  # Merge the simulated event times onto covariate data frame
  data <- merge(cov, dat)
  covs<-colnames(data)
...
#test<-data[-samp,-c(ncol(data))]
  ...
  ###########################
  final_mod_cb_glm <- fitSmoothHazard(status~bs(time)-time+z1+z2+z3+z1:time:time+z1:z3+z2:z3,
                                      data = train,
                                      time = "time",
                                      event="status",
                                      ratio = 100
 ...
 ##########################
  ...
  covars_output<-covars_input%>% #layer_batch_normalization()%>%
    layer_dense(units=layer1,use_bias = T,activation = activ)%>%
    layer_dropout(drpt)%>%
    layer_dense(units=layer2,use_bias = T,activation = activ)%>%
    layer_dropout(drpt)%>%
  ...
  fit<-fitSmoothHazSpline(pmnn,
                          epochs=epo,
                          batch_size=bsize,#floor(nrow(mod_cb_glm$data)/100),
                          verbose=0,
                          monitor="val_loss",
                          #val_split=0.2,
                          min_delta=min_delta,
  ...
  rm(pmnn,fit,annPreds,covars_input,covars_output)
  ######################
  ###coxph
  ######################
  ...
  coxnnsurv=fitDeepSurv(tiebreakTrain,fullTest,floor(nrow(train)/100),epochs=epo,valida=val,patience=patience,min_delta=min_delta,drpt=drpt,lay1=layer1,lay2=layer2,lay3=layer3,lay4=layer4,lr=lr)
  ...
  ######################
  source_python('src/deephitter.py')
  hitnnSurv=fitDeephit(train,fullTest,floor(nrow(train)/100),epo,valida=val,patience=patience,min_delta=min_delta,drpt=drpt,lay1=layer1,lay2=layer2,lay3=layer3,lay4=layer4,lr=lr)
...
trials<-1000000
numTests<-2
# paramTrials<-data.frame(#lr=runif(trials,0,0.1),
#                         #drpt=runif(trials,0,0.5),
#                         lr=sample(c(0.0001,0.001,0.01,0.1),replace=T),
#                         drpt=sample(c(0.1,0.2,0.3,0.4),replace=T),
#                         layer1=sample(seq(10,71,20), trials,replace = T),
#                         layer2=sample(seq(10,71,20), trials,replace = T),
#                         #layer3=sample(seq(1,100,25), trials,replace = T),
#                         bsize=sample(c(floor(nrow(mod_cb_glm$data)/100),floor(nrow(mod_cb_glm$data)/100/2)), trials,replace = T),
#                         actv=sample(c("relu","tanh",'selu'), trials,replace = T),
#                         integBrier=rep(NA,trials))
...
paramTrials<-as.data.frame(tidyr::crossing(lr=c(0.001,0.01,0.1),
                        ...
                        layer1=seq(10,71,25),
                        layer2=seq(10,71,25),
                        bsize=c(floor(nrow(mod_cb_glm$data)/100)),
                        actv=c("relu","tanh"),
                        integBrier=NA,
                        valLoss=NA))
# 
...
currentTime<-Sys.time() 
  ...
  covars_output<-covars_input%>% #layer_batch_normalization()%>%
    layer_dense(units=paramTrials$layer1[i],use_bias = T,activation = paramTrials$actv[i])%>%
    layer_dropout(paramTrials$drpt[i])%>%
    layer_dense(units=paramTrials$layer2[i],use_bias = T,activation = paramTrials$actv[i])%>%
    layer_dropout(paramTrials$drpt[i])%>%
    #layer_dense(units=paramTrials$layer3[i],use_bias = T,activation = paramTrials$actv[i])%>%
    #layer_dropout(paramTrials$drpt[i])%>%
    #layer_dense(units=layer4,use_bias = T,activation = 'relu')%>%
    #layer_dropout(drpt)%>%
    layer_dense(units=1,use_bias = T)
  ...
  pmnn<-pmnnModel(features=colnames(mod_cb_glm$data)[-ncol(mod_cb_glm$data)],
                  ...
                  offset=mod_cb_glm$offset,
                  timeVar = "time",
                  eventVar= "status",optimizer=optimizer_adam(learning_rate = paramTrials$lr[i])
  )
  
  fit<-fitSmoothHaz(pmnn,
                    epochs=epo,
                    batch_size=paramTrials$bsize[i],
                    verbose=0,
                    monitor="val_loss",
                    #val_split=0.2,
                    min_delta=min_delta,
  ...
  annPreds<-aar(fit,
                times=times,
                x_test=as.data.frame(val[,-c(ncol(data))])#rbind(train,val)
  )
  ...
  brierFinalResults <- Score(list('PMNN_Poly'=annProperpoly),
                             data =as.data.frame(val),#rbind(train,val) 
                             formula = Hist(time, status != 0) ~ 1, summary = c("risks","IPA","ibs"), 
                             se.fit = FALSE, metrics = "brier", contrasts = FALSE, times = times)$Brier$score
  cutted<-brierFinalResults[which(brierFinalResults$model=="PMNN_Poly"),]
  tempRes<-tempRes+mean(cutted[-c(1,2,24,25)]$IPA)/numTests
  tempVal<-tempVal+min(fit$resultOfFit$metrics$val_loss)/numTests
  }
  paramTrials$integBrier[i]<-tempRes
  paramTrials$valLoss[i]<-
  saveRDS(paramTrials,"complexTrials.rds")
  print(paste("trial",i))
  print(Sys.time()-currentTime)
  currentTime<-Sys.time()
