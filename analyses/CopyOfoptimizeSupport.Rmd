...
optimDeepSurv<-function(klist,test,times,baselineTrials,lr,drpt,layer1,layer2,bsize,actv){
  ...
  for(i in 1:nrow(paramTrials)){
    ...
    for(data in klist){
      ...
      scores<-na.omit(coxnnsurv$val_metrics$scores$loss$score)
      
      tempVal<-append(tempVal,scores[length(scores)])
...
optimDeepHit<-function(klist,test,times,baselineTrials,lr,drpt,layer1,layer2,bsize,actv,alp){
  ...
  for(i in 1:nrow(paramTrials)){
    ...
    for(data in klist){
      ...
      scores<-na.omit(hitnnSurv$val_metrics$scores$loss$score)
...
optimCBNN<-function(klist,test,times,lr,drpt,layer1,layer2,nBatch,actv){
  ...
  for(i in 1:nrow(paramTrials)){
    ...
    for(data in klist){
      ...
      train<-normalizer(train,means,sds,1)
      
      #train[,ncol(train)]<-data$train[,ncol(train)]
      val<-normalizer(val,means,sds,1)
      ...
      scores<-na.omit(fit$resultOfFit$metrics$val_loss)
...
while (pos <= iteration) {
  ...
  rm(fitDeepSurv)
    ...
    miniee <- Score(list('mod'=cnnCleaned),
                             ...
                             formula = Hist(time, status != 0) ~ 1, summary = c("risks","IPA","ibs"), 
                             se.fit = FALSE, metrics = "brier", contrasts = FALSE, times = times)$Brier$score
   miniee$IBS[which(miniee$model=="mod" & miniee$times==max(miniee$times))]
