...
#samp<-sample(seq(1,nrow(data),by=1), floor(nrow(data)*.80)) 
 ...
 spec = c(train = .70, test = .10, validate = .20)
...
sds<-sapply(trainOG, function(x) (sd(x)))
means<-sapply(trainOG, function(x) (mean(x)))
...
trainOG<-normalizer(res$train,means,sds)
...
#   colnames(cnnCleaned)<-seq(0,1,length.out = ncol(cnnCleaned))
  ...
  tiebreakTrain<-train
  tiebreakTrain$time<-tiebreakTrain$time+runif(nrow(tiebreakTrain),0,1/10000000)
  }
