...
```{r prostatePrep,eval=FALSE,hide=FALSE}

library(asaur)
prostate<-prostateSurvival


prostate<-prostate[-c(which(prostate$status==2)),]



preData<-data.frame(age=as.factor(prostate$ageGroup),
                    grade=as.factor(prostate$grade),
                    stage=as.factor(prostate$stage),
                    time=as.numeric(prostate$survTime),
                    status=as.numeric(prostate$status)
)
...
#saveRDS(res$test,"prostateTest.rds")
...
#curl -d "support done" ntfy.sh/updatejykiscripts
```
...
#saveRDS(winners,"prostateWins.rds")

```

```{r bestFits,finalFit}

winners<-readRDS("prostateWins.rds")
...
while (pos <= iteration) {
  ...
  fullTest<-readRDS("prostateTest.rds")
  ...
  saveRDS(BrierIPAList,'results/100prostateBrier.rds')
  saveRDS(cScore,'results/100prostatecidx.rds')
  saveRDS(AUClist,'results/100prostateAuc.rds')
  pos=pos+1
...
#BrierIPAList<-readRDS('results/100prostateBrier.rds')
...
AUClist<-readRDS('results/100prostateAuc.rds')
...
cidxList<-readRDS('results/100prostatecidx.rds')
