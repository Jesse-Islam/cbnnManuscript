...
###########################################  
 ...
 print('starting cox')
  ...
  covars_output<-covars_input%>% layer_batch_normalization()%>%
    ...
    layer_dropout(drpt)%>%
    #    layer_dense(units=25,use_bias = T,activation = 'relu')%>%
    #layer_dropout(0.2)%>%
    #    layer_dense(units=25,use_bias = T,activation = 'relu')%>%
    #layer_dropout(0.2)%>%
    layer_dense(units=1,use_bias = T)
  ...
  fit<-fitSmoothHaz(pmnn,
                    epochs=epo,
                    batch_size=bsize,#nrow(mod_cb_glm$data),
                    verbose=0,
                    monitor="val_loss",
                    #val_split=0.3,
                    min_delta=min_delta,
  ...
  ggplot(data=brierFinalResults$Brier$score, aes(x=times,y=IPA,col=model))+
  geom_line()+coord_cartesian(ylim=c(-0.5,0.5))
  fit$resultOfFit
  rm(pmnn,covars_output,covars_input,fit)
  ...
  et_train<-as.matrix(train)[,c(10,9)]
  et_test<-as.matrix(fullTest[,c(10,9)])
 ...
 saveRDS(BrierIPAList,'results/100gbsgBrier.rds')
  saveRDS(cScore,'results/100gbsgcidx.rds')
  print(pos)
  pos<-pos+1
}
