...
```{r myelomaPrep,eval=TRUE,hide=FALSE}
...
preData<-data.frame(year=as.numeric(myeloma$year),
                    entry=as.numeric(myeloma$entry),
                    time=as.numeric(myeloma$futime),
                    status=as.numeric(myeloma$death)
)
...
saveRDS(res$test,"cvData/myelomaTest.rds")
...
saveRDS(klist,"cvData/myelomaCV.rds")
...
klist<-readRDS("cvData/myelomaCV.rds")
...
winners<-list(cbnn=cbnnPerformance,
              ds=bestDS,
              dh=bestDH)
saveRDS(winners,"cvData/myelomaWins.rds")

```

```{r bestFits,finalFit}

winners<-readRDS("cvData/myelomaWins.rds")
...
while (pos <= iteration) {
  ...
  fullTest<-readRDS("cvData/myelomaTest.rds")
  
  if(sum(fullTest$time>max(train$time))>0){next }
  ...
  saveRDS(BrierIPAList,'results/100myelomaBrier.rds')
  saveRDS(cScore,'results/100myelomacidx.rds')
  saveRDS(AUClist,'results/100myelomaAuc.rds')
  pos=pos+1
...
BrierIPAList<-readRDS('results/100myelomaBrier.rds')
toRemove<-c()
cleaned<-list()
for(i in 1:length(BrierIPAList)){
  if(nrow(BrierIPAList[[i]])>110){
    cleaned[[length(cleaned)+1]]<-BrierIPAList[[i]]
  }
}

BrierIPAList<-cleaned
...
ipa+coord_cartesian(ylim=c(-0.8,0.4))
#end_time-start_time
...
AUClist<-readRDS('results/100myelomaAuc.rds')
...
cidxList<-readRDS('results/100myelomacidx.rds')
